...
def connection():
    enableTrace(True)
    ws = WebSocketApp("ws://ws-myserver.eu-gb.mybluemix.net/",
...
def on_open(ws):
    ...
    while True:
        ...
        while get_packets() < 4:
            time.sleep(0.02) # Задержка 20 мс
        for j in range(16):
            v1 += valuechannel4[-1][j]
            v2 += valuechannel2[-1][j]
            v3 += valuechannel3[-1][j]
            # v4 += valuechannel4[-1][j]
            # v5 += valuechannel5[-1][j]
            # v6 += valuechannel6[-1][j]
            # v7 += valuechannel7[-1][j]
            # v8 += valuechannel8[-1][j]    
        s = ""
        time.sleep(0.07)
        s += " ".join(map(str, v1)) + " XXX " + " ".join(map(str, v2)) + " YYY " + " ".join(map(str, v3))
        ws.send(s)
        del v1[:]
        del v2[:]
        del v3[:]
...
def lets_start():
    ...
    fileObject = open('losa132.txt','r')
    ds = pickle.load(fileObject)
    print 'ds_2', ds
    fileObject.close()
    os.remove('./' + 'losa132.txt')
    ...
    while True:  # Infinite loop
            ...
            background_label = Tk.Label(window_2, image=background_image)
            background_label.place(x=0, y=0, relwidth=1, relheight=1)
            window_2.wm_geometry("1000x650+20+40")
            window_2.update()
